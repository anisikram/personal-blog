...
---
Aujourd'hui je vous propose de plonger dans le fonctionnement de deux des collections les plus utilisées dans le monde Java. En effet lors de plusieurs discussions et entretiens que j'ai pu mener, je me suis rendu compte que leur fonctionnement intrinsèque pouvait être parfois mal compris par certains développeurs donc pourquoi ne pas écrire un article ou éclaircir tout cela ? 
...
Par exemple en faisant la somme des positions des lettres composant son nom dans l'alphabet : "Anis" donnerait donc $1 + 14 + 9 + 19 = 44$ mais pour être sûr qu'aucun nom ne dépasse les 100 (le nombre de tiroirs) on va appliquer une petite opération mathématique pour réduire le résultat $44\ \%\ 100 = 44$ .
Le courrier d'Anis sera systématiquement rangé et retrouvé dans le tiroir 44, l'employé n'aura plus qu'à faire ce simple calcul et ouvrir un seul tiroir optimisant drastiquement le temps employé à retrouver le courrier.
C'est ce qu'on appelle un accès direct, vous noterez ici que peu importe le nombre de clients, de courriers et de tiroirs,  l'employé n'en ouvrira toujours qu'un seul garantissant un temps constant donc $O(1)$.
...
Si vous ne redéfinissez pas ces méthodes, l'implémentation par défaut de la classe Object sera utilisée : 
 - `hashCode()`: Retournera un nombre aléatoire généré lors du premier appel et stocké dans l'en-tête de l'objet (une zone spéciale dans l'espace mémoire occupé par l'objet qui est utile à la JVM). On parle d'un "Identity Hashcode".
...
Mais comment ça s'organise dans le code ? 
...
Puis on fait un mélange de ces deux hash via l'opération $\oplus$ (XOR ou "OU EXCLUSIF" en bon français et `^` en java) : 
...
$(n - 1) \mathbin{\&} h$ 
où $n$ est le nombre de buckets, et ici il est égal à 16 donc $n - 1 = 15$ alors $000\ldots001111$ en binaire.
...
Pour la lecture, on défile les courriers dans le bon tiroir jusqu'à tomber sur le bon. À partir du bucket, on parcourt la liste chaînée pour trouver la clé qui va être égale via la méthode `equals` à la clé fournie en entrée pour enfin nous retourner la valeur contenue dans le couple clé-valeur. Vous comprenez mieux maintenant pourquoi la JVM définit un contrat entre `equals` et `hashcode`. 

Cependant comme vous pouvez le voir si notre bucket contient beaucoup d'éléments parcourir une liste chaînée peut s'avérer particulièrement long ($O(n)$) et faire s'effondrer les performances de notre collection. Mais qu'à cela ne tienne, les développeurs du SDK Java ont prévu le coup, nous allons maintenant voir comment.

### L'arbre rouge-noir

Dans le code de HashMap, on retrouve une constante nommée `int TREEIFY_THRESHOLD = 8`, elle détermine à partir de quelle taille le bucket va procéder à son arbrification (treefication en anglais). On passera donc d'une liste chaînée à un arbre comme structure de donnée dans un bucket. Pourquoi ?
...
Un bucket trop grand est délétère pour les performances si on a une liste chaînée puisque pour l'insertion et lecture, on a un temps algorithmique $O(n)$.  Tandis qu'un arbre binaire voit ces mêmes temps descendre à $O(\log(n))$. Cet arbre binaire est un arbre qu'on appelle rouge-noir, sans rentrer dans le détail il se maintient de lui-même équilibré lui permettant d'éviter de dériver vers une forme proche d'une liste chaînée et par la même occasion de se retrouver avec des performances $O(n)$.
...
Un arbre binaire a besoin de comparer la valeur contenue dans chaque nœud afin de maintenir un ordre permettant la recherche dichotomique. Alors comment les données sont triées ?
...
2. Si les hashcodes sont égaux et que les clés appartiennent à la même classe implémentant `Comparable`, on utilise `compareTo()`.
...
Ce 3ème cas garantit bien un ordre total et donc un arbre équilibré à l'insertion, mais il pose un problème à la **lecture**. Quand on cherche une clé via `get()`, l'arbre essaie de décider de quel côté descendre en comparant les hashcodes. Si plusieurs clés partagent le même hashcode et ne sont pas Comparable, HashMap ne peut plus se fier à l'ordre de secours pour trancher (deux clés distinctes avec le même hashcode peuvent se retrouver dans n'importe quelle branche selon l'ordre d'insertion). Elle est alors contrainte d'explorer les deux sous-arbres à la recherche d'une clé `equals()` à celle demandée, et les performances retombent vers  $O(n)$ dans le pire cas.

C'est pourquoi, sur un système critique, implémenter `Comparable` sur ses clés peut être intéressant. Mais avant cela, il faut d'abord se pencher sur la distribution des hashcodes pour éviter les collisions à la source.
...
Par défaut, cette valeur vaut **0,75**. Si 75 % des buckets sont occupés, un redimensionnement est déclenché.

$$\text{Seuil de déclenchement} = \text{Capacité actuelle} \times 0,75$$

Pour reprendre notre analogie, c'est comme si dès que 75 % de nos tiroirs sont occupés, on déménageait tous les courriers dans une nouvelle salle fraîchement construite avec le double de tiroirs. Et ce déménagement coûte cher car il faut instancier un nouveau tableau d'une capacité double de l'initial et redistribuer tous les éléments existants dans les nouveaux buckets. Autant vous dire qu'on veut éviter de faire ça tous les quatre matins.
